...
ncp_hdl_t inNcpHdl = NULL;
ncp_bool_t xgmac_flag = FALSE;
#endif
/*
...
OFDPA_ERROR_t ofdpaFlowAdd(ofdpaFlowEntry_t *flow)
{
  ...
  if (inNcpHdl == NULL)
   {
     LOG_ERROR("inNcpHdl is NULL, may not load the cfg file");
     return OFDPA_E_FAIL;
   }
  if (flow == NULL)
  {
      LOG_ERROR("flow is NULL");
      return OFDPA_E_FAIL;
  }
  switch(flow->tableId)
 {
    ...
    default:
        LOG_ERROR("Unsupportted table ID %d",flow->tableId);
        return OFDPA_E_FAIL;
 }
...
OFDPA_ERROR_t ofdpaFlowDelete(ofdpaFlowEntry_t *flow)
{
  ...
  if (flow->tableId == OFDPA_FLOW_TABLE_ID_BRIDGING)
      status == bridge_static_entry_delete(flow);
   if (flow->tableId == OFDPA_FLOW_TABLE_ID_VLAN_INGRESS)
      status == Ingress_Vlan_delete(flow);
   if (flow->tableId == OFDPA_FLOW_TABLE_ID_VLAN_EGRESS)
      status == Egress_Vlan_delete(flow);
  if (status == NCP_ST_SUCCESS)
     return OFDPA_E_NONE;
  else
     return OFDPA_E_INTERNAL;
...
OFDPA_ERROR_t ofdpaPortStateGet(uint32_t  portNum, uint32_t  *state)
{
    ...
    ncp_macChan_t macChan = NCP_GMAC0;
    if(rte_port_mchan_get(portNum,&macChan,xgmac_flag) != NCP_ST_SUCCESS)
      return OFDPA_E_INTERNAL;
    if(rte_port_link_status_get(portNum,xgmac_flag,&phyStatus) != NCP_ST_SUCCESS)
      return OFDPA_E_INTERNAL;
    state->config = phyStatus.configured;    
    //state->linkStatus = phyStatus.linkStatus;
    if(phyStatus.linkStatus == 0)
    {
        state->linkStatus |= OF_PORT_STATE_FLAG_LINK_DOWN;
    }
    state->speed = phyStatus.speed;
#endif
    return OFDPA_E_NONE;
}
...
OFDPA_ERROR_t ofdpaPortStatsGet(uint32_t portNum, ofdpaPortStats_t *stats)
{
    ...
    if(rte_port_mchan_get(portNum,&macChan,xgmac_flag) != NCP_ST_SUCCESS)
      return OFDPA_E_INTERNAL;
    if(rte_port_statistics_get(macChan,xgmac_flag,&tx_mac_stats,&rx_mac_stats) != NCP_ST_SUCCESS)
      return OFDPA_E_INTERNAL;
    stats->port_no = portNum;
    
    stats->rx_packets = (((uint64_t)rx_mac_stats.rx_frm.h)<<32&0xFFFFFFFF00000000)|rx_mac_stats.rx_frm.l;
    stats->rx_bytes   = (((uint64_t)rx_mac_stats.rx_octets.h)<<32&0xFFFFFFFF00000000)| rx_mac_stats.rx_octets.l;
    stats->rx_drops   = (((uint64_t)rx_mac_stats.rx_drop.h)<<32&0xFFFFFFFF00000000)| rx_mac_stats.rx_drop.l;
    stats->rx_errors  = (((uint64_t)rx_mac_stats.rx_bad_frm.h)<<32&0xFFFFFFFF00000000)| rx_mac_stats.rx_bad_frm.l;
    stats->collisions = (((uint64_t)rx_mac_stats.rx_carrier.h)<<32&0xFFFFFFFF00000000)| rx_mac_stats.rx_carrier.l;
...
/*********************************************************************
 * @purpose  the handler to read the config file
 *
 * @param    arg    
 * @param    buf
 * @param    len
...
int
ncp_configReadFnDefault(
    void *arg,
    void *buf,
    int len)
{
    return(fread((char *) buf, 1, len, (FILE *)arg));
}


/*********************************************************************
 * @purpose  load the rte config file
 *
...
int ncp_rte_load(void)
{
#if RTE_MODE
    ncp_st_t cfgStatus = NCP_ST_ERROR;
    FILE     *cfgFp = 0;
    ncp_uint32_t flags = 0;
    int     logicalDevNum = 0;
    ncp_uint32_t  devNum = 0;
    LOG_INFO("load cfg file \n");
    cfgFp = fopen("config.cfg", "rb");
    if(cfgFp == NULL ){
    ...
    if(cfgStatus!=NCP_ST_SUCCESS || inNcpHdl == NULL )
      {
        LOG_ERROR("\n ncpConfig Failed. Returned NULL Handle \n");
        return 1;
      }
    LOG_INFO("load cfg file sucessful\n");
    return cfgStatus;
#else
    return 0;
#endif
