                                  ...
                                  uint32_t *group_action_bitmap)
{
    ...
    OF_LIST_ACTION_ITER(actions, &act, rv) {
        ...
        case OF_ACTION_SET_FIELD: {
            ...
            //of_oxm_header_init(&oxm, act.version, 0, 1);
            //oxm.wire_object = act.wire_object;
            //oxm.wire_object.obj_offset += 4; /* skip action header */
            oxm.parent = &act;
        ...
        case OF_ACTION_SET_DL_SRC: 
            of_action_set_dl_src_dl_addr_get(&act, &group_bucket->srcMac);
            *group_action_bitmap |= IND_OFDPA_SRCMAC;
            break;

        case OF_ACTION_SET_DL_DST: 
            of_action_set_dl_dst_dl_addr_get(&act, &group_bucket->dstMac);
            *group_action_bitmap |= IND_OFDPA_DSTMAC;
            break;
        
        case OF_ACTION_SET_VLAN_VID: 
            of_action_set_vlan_vid_vlan_vid_get(&act, &vlanId);
            group_bucket->vlanId = vlanId;
            *group_action_bitmap |= IND_OFDPA_VLANID;
            break;
