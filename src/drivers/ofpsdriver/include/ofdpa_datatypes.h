...
typedef enum
{
  ...
  OFDPA_FLOW_TABLE_ID_VLAN_INGRESS      = 10,  /**< VLAN Table */
  ...
  OFDPA_FLOW_TABLE_ID_VLAN_EGRESS     = 70,  /**< ACL Table */
...
typedef struct ofdpaPolicyAclFlowEntry_s
{
  ...
  /** If non-zero, push/pop vlan ID */
  uint8_t     vlanAction;

  /** data for vlan action */
  uint16_t    VlanId;
...
typedef struct ofdpaFlowEntry_s
{
  ...
  union
  {
    ...
    /**Ingress VLAN Port Table Flow Entry */
    ofdpaVlanFlowEntry_t             IngressvlanFlowEntry;
    ...
    ofdpaPolicyAclFlowEntry_t        policyAclFlowEntry;
     ...
     /** Egress VLAN Port Table Flow Entry */
    ofdpaVlanFlowEntry_t             EgressvlanFlowEntry;
  } flowData;
...
typedef enum
{
  ...
  OFDPA_PORT_CONFIG_NO_RECV = 1 << 2,
  OFDPA_PORT_CONFIG_NO_FWD = 1 << 5,
  OFDPA_PORT_CONFIG_NO_PACKET_IN = 1 << 6
} OFDPA_PORT_CONFIG_t;
...
typedef enum
{
  ...
  OFDPA_PORT_STATE_BLOCKED = 1 << 1,
  OFDPA_PORT_STATE_LIVE = 1 << 2
} OFDPA_PORT_STATE_t;
...
typedef struct ofdpaPortLinkStatus
{
    OFDPA_PORT_STATE_t  linkStatus;
...
typedef struct ofdpaPortStats_s
{
  ...
  /** number of packets with RX Overrun Errors */
  ...
  /** Time port has been alive in nseconds */
  uint32_t duration_nseconds;
} ofdpaPortStats_t;
...
/*------------------------------------------------------------------------------------*/
/* Below define the whole ethernet packet, it is based on ivs swith code in file openvswit.h */
...
struct ovs_key_ipv6 {
	uint32_t ipv6_src[4];
	uint32_t ipv6_dst[4];
	uint32_t ipv6_label;	/* 20-bits in least-significant bits. */
	uint8_t   ipv6_proto;
	uint8_t   ipv6_tclass;
	uint8_t   ipv6_hlimit;
...
struct ovs_key_nd {
	...
	uint8_t  nd_sll[ETH_ALEN];
	uint8_t  nd_tll[ETH_ALEN];
};
...
/*Above define the whole ethernet packet, it is based on ivs swith code in file openvswit.h */
