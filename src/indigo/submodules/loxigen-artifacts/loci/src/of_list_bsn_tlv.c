...
int
of_list_bsn_tlv_first(of_list_bsn_tlv_t *list, of_object_t *obj)
{
    int rv;

    of_bsn_tlv_init(obj, list->version, -1, 1);

    if ((rv = of_list_first(list, obj)) < 0) {
        return rv;
    }

    of_bsn_tlv_wire_object_id_get(obj, &obj->object_id);
...
int
of_list_bsn_tlv_next(of_list_bsn_tlv_t *list, of_object_t *obj)
{
    int rv;
...
int
of_list_bsn_tlv_append_bind(of_list_bsn_tlv_t *list, of_object_t *obj)
{
    return of_list_append_bind(list, obj);
}

/**
 * Append an object to a of_list_bsn_tlv list.
 *
...
int
of_list_bsn_tlv_append(of_list_bsn_tlv_t *list, of_object_t *obj)
{
...
of_list_bsn_tlv_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_BSN_TLV];
...
void
of_list_bsn_tlv_init(of_object_t *obj,
