...
int
of_list_port_stats_prop_first(of_list_port_stats_prop_t *list, of_object_t *obj)
{
    int rv;

    of_port_stats_prop_init(obj, list->version, -1, 1);

    if ((rv = of_list_first(list, obj)) < 0) {
        return rv;
    }

    of_port_stats_prop_wire_object_id_get(obj, &obj->object_id);
...
int
of_list_port_stats_prop_next(of_list_port_stats_prop_t *list, of_object_t *obj)
{
    int rv;
...
/**
 * Set up to append an object of type of_port_stats_prop to an of_list_port_stats_prop.
...
int
of_list_port_stats_prop_append_bind(of_list_port_stats_prop_t *list, of_object_t *obj)
{
    return of_list_append_bind(list, obj);
}

/**
 * Append an object to a of_list_port_stats_prop list.
 *
...
int
of_list_port_stats_prop_append(of_list_port_stats_prop_t *list, of_object_t *obj)
{
    return of_list_append(list, obj);
}

/**
 * \defgroup of_list_port_stats_prop of_list_port_stats_prop
 */

/**
 * Create a new of_list_port_stats_prop object
 *
...
of_list_port_stats_prop_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_PORT_STATS_PROP];

    if ((obj = of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }

    of_list_port_stats_prop_init(obj, version, bytes, 0);

    return obj;
}

/**
 * Initialize an object of type of_list_port_stats_prop.
 *
...
void
of_list_port_stats_prop_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_PORT_STATS_PROP] >= 0);
    if (clean_wire) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_LIST_PORT_STATS_PROP;
