...
int
of_list_table_desc_first(of_list_table_desc_t *list, of_object_t *obj)
{
    int rv;

    of_table_desc_init(obj, list->version, -1, 1);
...
int
of_list_table_desc_next(of_list_table_desc_t *list, of_object_t *obj)
{
    int rv;
...
/**
 * Set up to append an object of type of_table_desc to an of_list_table_desc.
...
int
of_list_table_desc_append_bind(of_list_table_desc_t *list, of_object_t *obj)
{
    return of_list_append_bind(list, obj);
}

/**
 * Append an object to a of_list_table_desc list.
 *
...
int
of_list_table_desc_append(of_list_table_desc_t *list, of_object_t *obj)
{
    return of_list_append(list, obj);
}

/**
 * \defgroup of_list_table_desc of_list_table_desc
 */

/**
 * Create a new of_list_table_desc object
 *
...
of_list_table_desc_new(of_version_t version)
{
    ...
    bytes = of_object_fixed_len[version][OF_LIST_TABLE_DESC];

    if ((obj = of_object_new(OF_WIRE_BUFFER_MAX_LENGTH)) == NULL) {
        return NULL;
    }

    of_list_table_desc_init(obj, version, bytes, 0);

    return obj;
}

/**
 * Initialize an object of type of_list_table_desc.
 *
...
void
of_list_table_desc_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_TABLE_DESC] >= 0);
    if (clean_wire) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_LIST_TABLE_DESC;
