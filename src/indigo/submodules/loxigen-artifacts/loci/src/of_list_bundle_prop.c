...
int
of_list_bundle_prop_first(of_list_bundle_prop_t *list, of_object_t *obj)
{
    int rv;

    of_bundle_prop_init(obj, list->version, -1, 1);

    if ((rv = of_list_first(list, obj)) < 0) {
        return rv;
    }

    of_bundle_prop_wire_object_id_get(obj, &obj->object_id);
...
int
of_list_bundle_prop_next(of_list_bundle_prop_t *list, of_object_t *obj)
{
    int rv;
...
/**
 * Set up to append an object of type of_bundle_prop to an of_list_bundle_prop.
...
int
of_list_bundle_prop_append_bind(of_list_bundle_prop_t *list, of_object_t *obj)
{
    return of_list_append_bind(list, obj);
}

/**
 * Append an object to a of_list_bundle_prop list.
 *
...
int
of_list_bundle_prop_append(of_list_bundle_prop_t *list, of_object_t *obj)
{
    return of_list_append(list, obj);
}

/**
 * \defgroup of_list_bundle_prop of_list_bundle_prop
 */

/**
 * Create a new of_list_bundle_prop object
 *
...
/**
 * Initialize an object of type of_list_bundle_prop.
 *
...
void
of_list_bundle_prop_init(of_object_t *obj,
    of_version_t version, int bytes, int clean_wire)
{
    LOCI_ASSERT(of_object_fixed_len[version][OF_LIST_BUNDLE_PROP] >= 0);
    if (clean_wire) {
    ...
    obj->version = version;
    obj->length = bytes;
    obj->object_id = OF_LIST_BUNDLE_PROP;
